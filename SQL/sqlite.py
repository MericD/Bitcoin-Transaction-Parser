...
def addOP(connection, block_number, transaction_id, tx_value, op_return, op_dec, op_length, tx_address, address_number):
    cursor = connection.cursor()
    print("Add trans to table")
    __execute_command(cursor,qy.get_add_filtered_OP( transaction_id,block_number,  tx_value, op_return, op_dec, op_length, tx_address, address_number))
    # never forget this, if you want that the changes are saved
    connection.commit()
