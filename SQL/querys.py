...
# tx_time - transaction time stamp
...
__add_tx_qy = """INSERT INTO tx 
    (transaction_id, version, tx_size ,vin_size, vout_size,  tx_value, op_return, block_number) 
    VALUES 
    ("{transaction_id}", {version}, {tx_size}, {vin_size}, {vout_size}, "{tx_value}", "{op_return}", {block_number})
;"""
...
def get_add_tx_query(tx_id, tx_v, tx_s ,tx_vin_size, tx_vout_size, tx_val, tx_op_return, tx_block_id):
    return __add_tx_qy.format(transaction_id=tx_id, version=tx_v, tx_size=tx_s, vin_size=tx_vin_size, vout_size=tx_vout_size, tx_value=tx_val, op_return=tx_op_return, block_number=tx_block_id) 
